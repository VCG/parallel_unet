    ...
    def implement_usknet(self, netconf, net, run_shape, blobs, fmaps_start, fmaps_end): 
        ...
        for uidx in range(0,len(netconf.u_netconfs)):
            ...
            if unetconf.unet_depth > 0:
                ...
                for i in range(0, unetconf.unet_depth):
                    ...
                    for j in range(0,len(convolution_config)):
                        ...
                        blobs = blobs + [relu]
                        print('c:',j, 'n:', conv.name, 'k:',convolution_config[j])
            ...
            if (unetconf.unet_depth > 0 and (len(unetconf.sk_netconfs) - 1 < unetconf.unet_depth or unetconf.sk_netconfs[unetconf.unet_depth] == None)):
                ...
                print('normal conv...')
                for j in range(0,len(convolution_config)):
                    print('conv:', j, 'k:', convolution_config[j])
            ...
            if unetconf.unet_depth > 0:
                ...
                for i in range(0, unetconf.unet_depth):
                    ...
                    for j in range(0,len(convolution_config)):
                        print('d:', j, 'k:', convolution_config[j])
...
def implement_parallel_unets(netconf, netgen, net, net_mode):
    ...
    loss_flag = None
    if net_mode == caffe_pb2.TRAIN:
        fusion_blobs = L.Python( *net_heads, ntop=4, name='Fusion', python_param={'module':'fusion_layer', 'layer':'FusionLayer', 'param_str':param_json })
        loss_flag = fusion_blobs[-1]
        loss_flag.name = 'enable'
        cov_inputs = [fusion_blobs[-4], fusion_blobs[-3], fusion_blobs[-2]]
        blobs = blobs + [loss_flag]
    else:
        fusion_blobs = L.Python( *net_heads, ntop=3, name='Fusion', python_param={'module':'fusion_layer', 'layer':'FusionLayer', 'param_str':param_json })
        cov_inputs = [fusion_blobs[-3], fusion_blobs[-2], fusion_blobs[-1]]

    blobs = blobs + cov_inputs
    dims = 3
    run_shape.shape = netconf.input_shape
    run_shape.dilation = [1 for i in range(0,dims)]
    run_shape.fmaps = 1
    ...
    convolutions = []
    for name, cov_input in zip(net_names, cov_inputs):
        last_blob = netgen.convolution(run_shape_out, cov_input, fmaps, kernel_size=[1])
        last_blob.name = 'cov_%s'%(name)
        blobs = blobs + [last_blob]
        convolutions.append( last_blob )

    concat = L.Concat( *convolutions )
    blobs = blobs + [concat]
    return blobs, loss_flag
...
def caffenet(netconf, netmode):
    ...
    if netmode == caffe_pb2.TEST:
        ...
        net_blobs, loss_flag = implement_parallel_unets(netconf, netgen, net, netmode)
        blobs = blobs + net_blobs
        last_blob = blobs[-1]
...
#            net.label, net.labeli = netgen.data_layer([1]+[netconf.fmap_output]+netconf.output_shape)
        ...
        if netconf.loss_function == 'euclid':
            ...
            net.loss = L.GatedEuclideanLoss(last_blob, net.label, net.scale, loss_flag, ntop=0)
