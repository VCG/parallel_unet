...
void ForwardIters_NoGIL(Net<Dtype>& net, int_tp iters) {
  Py_BEGIN_ALLOW_THREADS
  for (int_tp i=0; i<iters; i++) {
    net.Forward();
  }
...
// NOLINT_NEXT_LINE(runtime/references)
Dtype StepParallel_NoGIL(Solver<Dtype>& solver, int_tp iters) {
  Dtype smoothed_loss;
  Py_BEGIN_ALLOW_THREADS
  smoothed_loss = solver.StepParallel(iters);
  Py_END_ALLOW_THREADS
  return smoothed_loss;
}

// NOLINT_NEXT_LINE(runtime/references)
Dtype StepForward_NoGIL(Solver<Dtype>& solver, int_tp iters) {
  Dtype smoothed_loss;
  Py_BEGIN_ALLOW_THREADS
  smoothed_loss = solver.StepForward(iters);
  Py_END_ALLOW_THREADS
  return smoothed_loss;
}

// NOLINT_NEXT_LINE(runtime/references)
void StepBackward_NoGIL(Solver<Dtype>& solver, int_tp iters) {
  Py_BEGIN_ALLOW_THREADS
  solver.StepBackward(iters);
  Py_END_ALLOW_THREADS
}
...
BOOST_PYTHON_MODULE(_caffe) {
  ...
  bp::class_<Net<Dtype>, shared_ptr<Net<Dtype> >, boost::noncopyable >("Net",
    ...
    .def("_forward_iters", &ForwardIters_NoGIL)
  ...
  bp::class_<Solver<Dtype>, shared_ptr<Solver<Dtype> >, boost::noncopyable>(
    ...
    .def("stepParallel", &StepParallel_NoGIL)
    .def("stepForward", &StepForward_NoGIL)
    .def("stepBackward", &StepBackward_NoGIL)
    .def("step", &Step_NoGIL)
