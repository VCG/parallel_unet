...
void Net<Dtype>::AppendParam(const NetParameter& param, const int_tp layer_id,
  ...
  LOG(INFO) << "===>adding param id: " << param_id << " name: " << param_name;
...
void Net<Dtype>::Backward() {
  ...
  //if (debug_info_) {
    ...
    for (int_tp i = 0; i < learnable_params_.size(); ++i) {
      ...
      const string& param_display_name = param_display_names_[i];

      //LOG(INFO) << "param: " <<  param_display_name
      //          << " data: " <<  learnable_params_[i]->asum_data()
      //          << " diff: " << learnable_params_[i]->asum_diff(); 
    }
